...
namespace ECommerceDotNet.Presentation.Api.Controllers
{
    ...
    public class CartController : ApiControllerBase
    {
        ...
        #region Update Cart
        [HttpPut("{id}")]
        public async Task<ActionResult<bool>> Update([FromBody] CartRequestDto cartRequestDto, string id)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest();
            ...
            bool state = await _cartService.UpdateCartAsync(cartRequestDto, id);
            if (state == true)
            {
                return Ok(state);
            }
        ...
        public async Task<ActionResult<int>> Delete(string id)
        {
            ...
            bool state = await _cartService.DeleteCartAsync(id);
            if (state == true)
            {
                return Ok(state);
            }
